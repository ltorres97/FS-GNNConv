...
class Meta_eval(nn.Module):
    def __init__(self, args):
        super(Meta_eval,self).__init__()
        ...
        #for the example on main_evaluate.py for transfer learning consider:
        ...
        print(count_parameters(self.cnn))
    ...
    def meta_evaluate(self):  
        ...
        print(sum(p.numel() for p in self.cnn.parameters() if p.requires_grad))
        print(self.cnn.parameters)
                
        roc_scores = []
        p = parameters_to_vector(self.gnn.parameters())
        ...
        for test_task in range(self.test_tasks):
            ...
            vector_to_parameters(p, self.gnn.parameters())
       
        """Uncomment the last line for transfer learning experiments"""
        return roc_scores, self.gnn.state_dict(), self.cnn.state_dict(), self.optimizer.state_dict(), self.meta_optimizer.state_dict()
        #return [statistics.mean(roc_scores)], self.gnn.state_dict(), self.cnn.state_dict(), self.optimizer.state_dict(), self.meta_optimizer.state_dict()
