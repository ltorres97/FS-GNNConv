        ...
        for k,j  in enumerate(file):
            ...
            for i in range(1,28):
                ...
                if i not in T:
                    T[i] = [[],[]]
                if sample[i] == "0" or "0\n":
                    T[i][0].append(sample[0].rstrip())
                elif sample[i] == "1" or "1\n":
                    T[i][1].append(sample[0].rstrip())
      
        for t in T:
            d = 'Data/' + data + "/pre-processed/" + "task_" +str(t)
            os.makedirs(d, exist_ok=True)
            os.makedirs(d + "/raw", exist_ok=True)
            os.makedirs(d + "/processed", exist_ok=True)
            
            with open(d + "/raw/" + data + "_task_" + str(t), "wb") as fp:   #Pickling
                pickle.dump(T[t], fp)
