...
class Meta_eval(nn.Module):
    ...
    def meta_evaluate(self):
        ...
        for test_task in range(self.test_tasks):
            ...
            for k in range(0, self.k_test):
                ...
                for batch_idx, batch in enumerate(tqdm(support_set, desc="Iteration")):
                    ...
                    graph_loss += torch.sum(self.criterion(graph_pred.double(), label))/graph_pred.size()[0]
        ...
        #return [statistics.mean(roc_scores)], self.gnn.state_dict(), self.optimizer.state_dict()